...
func newPath(action messages.Action) path {
...
func (r *Rt) invokeHandleFunc(info *Context, data interface{}) error {
	...
	if r.hasParam {
		...
		if r.shouldValidate {
			...
			m, ok := data.(*messages.GlideMessage)
			if !ok {
...
func (r *Router) Handle(uid int64, device int64, msg *messages.GlideMessage) (*messages.GlideMessage, error) {
	ctx := &Context{
		Uid:    uid,
		Seq:    msg.GetSeq(),
		Device: device,
		Action: string(msg.GetAction()),
	}
	p := newPath(msg.GetAction())

	respCh := make(chan *messages.GlideMessage)
	errCh := make(chan error)
	timeout := time.After(time.Second * 3)

	go func() {
		ctx.R = func(message *messages.GlideMessage) {
			ctx.done = true
			respCh <- message
		}
